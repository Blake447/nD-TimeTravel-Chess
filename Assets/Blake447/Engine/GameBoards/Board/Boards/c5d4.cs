...
public class c5d4 : Board
{
    int[] board_dimensions = new int[] { 4, 4, 4, 4, 4 };
    ...
    public override int[] PositionToCoordinate(Vector3 position)
    {
        ...
        offset = offset / 8.0f;
        int offset_u = Mathf.FloorToInt(offset.x);
        offset = local_offset;
        offset = offset + new Vector3(1, 1, 0);
        offset = offset / 6.0f;
        ...
        offset = local_offset;
        offset = offset - Vector3.right * 8.0f * offset_u;
        offset = offset - Vector3.up * 6.0f * offset_w;
        offset = offset - Vector3.forward * 4.0f * offset_z;
        ...
        int[] coordinate = new int[5] { offset_x, offset_y, offset_z, offset_w, offset_u };
    ...
    public override Vector3 CoordinateToPosition(int[] coordinate)
    {
        ...
        position = position - new Vector3(2.0f, 2.0f);
        position = position * (1.0f - z * 0.1f);
        position = position + new Vector3(2.0f, 2.0f);
        position = position + z * new Vector3(0, 0, 4);
        position = position + w * new Vector3(0, 6, 0);
        position = position + u * new Vector3(8, 0, 0);
    ...
    public override Vector3 SnapCamera(Vector3 position)
    {
        ...
        if (coordinate != null)
        {
            ...
            Vector3 b = CoordinateToPosition(new int[5] { 3, 3, coordinate[2], coordinate[3], coordinate[4] });
            Vector3 m = (a + b) * 0.5f;
            return m;
        }
        return position;
    ...
    public override Vector3 GetCenter()
    {
        ...
        Vector3 a2 = CoordinateToPosition(new int[] { 3, 3, 3, 3, 3 });
