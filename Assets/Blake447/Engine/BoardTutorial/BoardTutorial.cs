...
public class BoardTutorial : MonoBehaviour
{
    ...
    int current_state = 0;
    [SerializeField] TutorialElement[] tutorialElements;

    public void AssignIDs()
    {
        for (int i = 0; tutorialElements != null && i < tutorialElements.Length; i++)
            tutorialElements[i].SetClient(this, i);
    }
    private void Awake()
    {
        AssignIDs();
        tutorialElements[0].Activate();
    }
    public void AdvanceGameState()
    {
        if (tutorialElements != null && current_state < tutorialElements.Length && tutorialElements[current_state] != null)
            tutorialElements[current_state].Deactivate();
        current_state++;
        if (tutorialElements != null && current_state < tutorialElements.Length && tutorialElements[current_state] != null)
            tutorialElements[current_state].Activate();
    }
